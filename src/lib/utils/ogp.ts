...
export const fetchOgpContent = async (urlString: string): Promise<Ogp | null> => {
	try {
		const response = await fetch(`/api/ogp?url=${encodeURIComponent(urlString)}`).catch((err) =>
			console.log(err)
		);
		...
		return {
			title: (result.open_graph && result.open_graph.title) || result.title || '',
			image:
				result.open_graph && result.open_graph.images
					? result.open_graph.images[result.open_graph.images.length - 1].url
					: '',
			description: (result.open_graph && result.open_graph.description) || result.description || '',
			favicon: result.favicon || '',
			audio: result.open_graph?.audio
				? result.open_graph.audio[result.open_graph.audio.length - 1]?.url
				: result.twitter_card?.players && Array.isArray(result.twitter_card.players)
					? result.twitter_card.players[result.twitter_card.players.length - 1]?.stream || ''
					: '',
			video:
				result.open_graph &&
			...
			player:
				...
				result.twitter_card.players.length > 0
					? result.twitter_card.players[result.twitter_card.players.length - 1]?.url || ''
					: '',
			playerStream:
				result.twitter_card?.players &&
				Array.isArray(result.twitter_card.players) &&
