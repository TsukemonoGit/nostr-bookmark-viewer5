...
import { queryClient } from './stores.svelte';
...
export async function makeInvoice({
	...
	try {
		...
		const signedRequest = await (window.nostr as Nostr.Nip07.Nostr)?.signEvent(zapRequest);
...
export const getZapLNURLPubkey = async (
	...
	if (!client) return { pub: undefined, error: 'query client error' };
	const data: { pub: string | undefined; error?: string } | undefined = client.getQueryData([
		'zapLNURLPubkey',
		metadata.pubkey
	]);
	if (data) {
...
export async function getNurlFetch(lnurl: string): Promise<any | undefined> {
	...
	const data: Response | undefined = client.getQueryData(['fetchNnurl', lnurl]);
	if (data) return data;
...
export async function getZapRelay(pubkey: string): Promise<string[]> {
   ...
   if(!client) return []
	...
	let queryRelay: EventPacket | undefined = client.getQueryData(['defaultRelay', pubkey]);
	if (!queryRelay) {
		...
		if (relayData.length > 0) {
			queryClient.setQueryData(['relays', pubkey], (oldData: any) => relayData[0]);
...
export function extractAmount(
	...
	try {
		...
		//console.log(decoded);
		if (decoded) {
			const amountSection = decoded.sections.find((section) => section.name === 'amount')?.value;
			//  console.log("zapRequestEvent", zapRequestEvent);
			// console.log("amountSection", amountSection);

			const requestAmount = zapRequestEvent?.tags.find((tag) => tag[0] === 'amount')?.[1];
...
export function lnurlToZapAddress(lud06: string): string | undefined {
	try {
		...
		const match = lnurl.match(/^https:\/\/([^\/]+)\/\.well-known\/lnurlp\/([^\/]+)$/);
...
//lud16( name@domain )優先
export async function getZapEndpoint(metadata: Nostr.Event): Promise<null | string> {
	try {
		let lnurl: string = '';
		let { lud06, lud16 } = JSON.parse(metadata.content);
